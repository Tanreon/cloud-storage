...
public class CommandDeleteFileAction extends AbstractAction {
    ...
    public CommandDeleteFileAction(ChannelHandlerContext ctx, ByteBuf byteBuf, Auth auth) {
        this.ctx = ctx;
    ...
    protected void process() throws ProcessException {
        Path filePath = Paths.get(Server.STORAGE_PATH, this.auth.getLogin(), this.fileName);
